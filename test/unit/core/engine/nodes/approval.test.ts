...
describe('ApprovalNode', () => {
  ...
  describe('double-resolve guard', () => {
    it('silently discards the second resolve call and preserves the first result', async () => {
      const emitter = createEngineEmitter();
      let capturedResolve: ((result: ApprovalResult) => void) | undefined;
      ...
      capturedResolve!({ approved: true }); // first call — settles the Promise
      capturedResolve!({ approved: false }); // second call — silently discarded
  ...
  describe('no-listener guard', () => {
    it('rejects with EngineError when no approval_requested listener is registered', async () => {
  ...
  describe('ApprovalNode.parse', () => {
    it('throws a validation error when required approval.message is missing', () => {
      expect(() => ApprovalNode.parse({ id: 'a1', approval: {} })).toThrow();
    });
  });
});
